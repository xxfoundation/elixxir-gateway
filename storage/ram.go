...
type MapBuffer struct {
	messageCollection map[userid.UserID]map[string]*pb.CmixMessage
	messageIDs        map[userid.UserID][]string
...
func NewMessageBuffer() MessageBuffer {
	...
	buffer := MessageBuffer(&MapBuffer{
		messageCollection: make(map[userid.UserID]map[string]*pb.CmixMessage),
		messageIDs:        make(map[userid.UserID][]string),
...
func (m *MapBuffer) GetMessage(userID *userid.UserID,
...
func (m *MapBuffer) GetMessageIDs(userID *userid.UserID, messageID string) (
	[]string, bool) {
	m.mux.Lock()
...
func (m *MapBuffer) DeleteMessage(userID *userid.UserID, msgID string) {
	m.mux.Lock()
...
func (m *MapBuffer) AddMessage(userID *userid.UserID, msgID string,
	msg *pb.CmixMessage) {
	...
	if (len(m.messageIDs[*userID]) + 1) > MaxUserMessagesLimit {
		...
		defer func(m *MapBuffer, userID *userid.UserID, msgIDs []string) {
			for i := range msgIDs {
