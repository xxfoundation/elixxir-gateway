...
func (gw *Instance) gossipRateLimitReceive(msg *gossip.GossipMsg) error {
	...
	jww.INFO.Printf("Received rate limit for round %d with %d senders and %d ips", payloadMsg.RoundID, len(payloadMsg.SenderIds), len(payloadMsg.Ips))
...
func (gw *Instance) GossipBatch(round id.Round, senders []*id.ID, ips []string) error {
	...
	jww.INFO.Printf("Sending rate limit for round %d with %d senders and %d ips", round, len(senders), len(ips))
