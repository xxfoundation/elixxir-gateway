...
func TestInstance_GossipVerify(t *testing.T) {
	...
	_, err = gatewayInstance.Comms.AddHost(originId, "", gatewayCert, params)
	if err != nil {
...
func TestInstance_StartPeersThread(t *testing.T) {
	...
	params := connect.GetDefaultHostParams()
	params.AuthEnabled = false
	_, err = gatewayInstance.Comms.AddHost(gwId, "0.0.0.0", gatewayCert, params)
	if err != nil {
		t.Errorf("Unable to add test host: %+v", err)
	}
...
func TestInstance_GossipBatch(t *testing.T) {
	...
	params := connect.GetDefaultHostParams()
	params.AuthEnabled = false
	_, err = gatewayInstance.Comms.AddHost(gatewayInstance.Comms.Id, GW_ADDRESS, gatewayCert, params)
	if err != nil {
		t.Errorf("Unable to add test host: %+v", err)
	}
	protocol, exists := gatewayInstance.Comms.Manager.Get(RateLimitGossip)
